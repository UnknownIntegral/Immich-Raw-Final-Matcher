        ...
        return """
                ...
                <body>
                  ...
                  <script>
                    ...
                    function scoreBreakdown(details=[]){const rows=(details||[]).filter(detail=>!['captureTimestamp','cameraType','lensModel','fNumber','focalLength','iso','exposureTime','modifiedTimestamp'].includes(detail.key));if(!rows.length)return '';return `<div class="note">${rows.map(detail=>`${escapeHtml(detail.label)}: ${escapeHtml(detail.points)}/${escapeHtml(detail.weight)} (${escapeHtml(detail.note)})`).join(' | ')}</div>`;}
