...
<Container version="2">
  ...
  <Config Name="RAW Immich API Key" Target="RAW_IMMICH_API_KEY" Default="" Mode="" Description="Required API key for RAW-account discovery, thumbnails, and RAW-side decision tags." Type="Variable" Display="always" Required="true" Mask="true"/>
  <Config Name="Final Immich API Key" Target="FINAL_IMMICH_API_KEY" Default="" Mode="" Description="Required API key for final-account discovery, thumbnails, and final-side decision tags." Type="Variable" Display="always" Required="true" Mask="true"/>
...
Set `RAW_IMMICH_API_KEY` to the RAW user's key and `FINAL_IMMICH_API_KEY` to the final-image user's key. Both are required; the container does not accept a shared or admin Immich API key.
